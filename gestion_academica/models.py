...
class Alumno(models.Model):
    ...
    datos_pase = models.TextField(
        blank=True, 
        null=True, 
        help_text="Descripción o motivos de la baja (ej. escuela de destino, razones personales, etc.)."
    )
...
class AsignacionCargo(models.Model):
    SITUACIONES_REVISTA = [
        ...
        ('INTERINO', 'Interino'),
        ('SUPLENTE', 'Suplente'),
        ('PROVISIONAL', 'Provisional'),  # Corregido ortografía administrativa
    ]
...
class Dictado(models.Model):
    ...
    grupo = models.CharField(
        max_length=10, 
        choices=OPCIONES_GRUPO, 
        default='UNICO',
        verbose_name="Grupo de Dictado"
    )
    ...
    class Meta:
        # Se incluye 'grupo' para permitir múltiples dictados de la misma materia en el mismo curso
    ...
    def __str__(self):
        ...
        grupo_str = f" [{self.get_grupo_display()}]" if self.grupo != 'UNICO' else ""
        return f"{self.materia.nombre}{grupo_str} - {self.curso} ({self.ciclo_lectivo}){profe_str}"
...
class NotaEtapa(models.Model):
    ...
    valoracion_previa = models.CharField(
        max_length=3,
        choices=CONCEPTUAL_CHOICES,
        null=True,
        blank=True,
        verbose_name="Valoración previa"
    )
